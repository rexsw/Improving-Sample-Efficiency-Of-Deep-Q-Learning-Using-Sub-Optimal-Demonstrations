...
class ReplayBuffer(object):
	...
	def add(self, s0, a, r, s1, done):
		...
		if done:
			...
			#score = self.pclass.actor(inputarray).cpu().detach().numpy()
...
#pclass.actor.load_state_dict(torch.load( './classifier_LunarLander-v2DQN baseline.pth'))

missclass = pclass.update(train, test, config.env + experment)

agent = DDQNAgent(config,pclass)
experment = " DQN classifier 5050 oldest"
plt.figure(0)
...
for i in range(5):
	...
	p2 = axs[1,0].plot(np.arange(len(deleted)), deleted)
